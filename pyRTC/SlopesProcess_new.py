...
class SlopesProcess(pyRTCComponent):
    ...
    def __init__(self, conf, force_name_camera="wfs") -> None:

        self.confWFS = conf[force_name_camera]
        self.name = "Slopes"
        ...
        self.wfsMeta = ImageSHM(f"{force_name_camera}_meta", (ImageSHM.METADATA_SIZE,), np.float64).read_noblock_safe()
        self.imageDType = float_to_dtype(self.wfsMeta[3])
        #self.wfsShm = ImageSHM(force_name_camera, (48, self.imageShape[0],self.imageShape[1]) , self.imageDType)
        self.wfsShm = ImageSHM(force_name_camera, (self.imageShape[0],self.imageShape[1]) , self.imageDType)
    ...
    def readImage(self):
        return np.sum(self.wfsShm.read(), axis=0)
    ...
    def setPupils(self, pupilLocs, pupilRadius):
        ...
        if self.signalType == "slopes":
            #self.signalSize = np.count_nonzero(self.pupilMask_per[0,:,:])*2
            self.signalSize = np.count_nonzero(self.pupilMask_per[:,:])*2

#            slopemask =  self.pupilMask_per[0, self.pupilLocs[0][1]-self.pupilRadius+1:self.pupilLocs[0][1]+self.pupilRadius, 

            slopemask =  self.pupilMask_per[self.pupilLocs[0][1]-self.pupilRadius+1:self.pupilLocs[0][1]+self.pupilRadius, 
    ...
    #     pupils = []
    #     #self.pupilMask_per = np.zeros((4, self.imageShape[0], self.imageShape[1]))
    #     self.pupilMask_per = np.zeros(( self.imageShape[0], self.imageShape[1]))
    #     xx,yy = np.meshgrid(np.arange(self.pupilMask_per.shape[1]),np.arange(self.pupilMask_per.shape[0]))
    #     for i, pupil_loc in enumerate(self.pupilLocs):
    #         px, py = pupil_loc
    #         zz = np.sqrt((xx-px)**2 + (yy-py)**2)
    #         pupils.append(zz < self.pupilRadius)
    #         self.pupilMask_per[:,:] += pupils[-1]*(i+1)
    #     self.p1mask = self.pupilMask_per[:,:] == 1
    #     self.p2mask = self.pupilMask_per[:,:] == 2
    #     self.p3mask = self.pupilMask_per[:,:] == 3
    #     self.p4mask = self.pupilMask_per[:,:] == 4
    #     #self.pupilMask = np.sum(self.pupilMask_per, axis=0)
    #     self.pupilMask = self.pupilMask_per
    ...
    def computePupilsMask(self):
        ...
        self.pupilMask_per = self.pupilMask
        return
