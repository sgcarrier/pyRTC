...
class RealTimeView(QMainWindow):
    def __init__(self, shm_name, fps, static_vmin=None, static_vmax=None):
        ...
        shm_frames, shm_width, shm_height = int(metadata[4]),  int(metadata[5]), int(metadata[6])

        shm_height = max(1,shm_height)
        shm_width = max(1,shm_width)
        shm_frames = max(1,shm_frames)

        shm_dtype = float_to_dtype(metadata[3])
        self.shm = ImageSHM(shm_name, (shm_frames, shm_width, shm_height), shm_dtype)
        ...
        cube = self.shm.read_noblock_safe()
        frame = np.sum(cube, axis=0)
